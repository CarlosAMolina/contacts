...
async fn main() -> Result<(), Error> {
    ...
    test_get_contacts_searchs_all_columns(&url_api).await;
...
async fn test_get_contacts_searchs_all_columns(url_api: &String) {
    ...
    let column_values = vec![
        "only in column user_name".to_string(),
        "only in column user_surname".to_string(),
        "only in column nicknames".to_string(),
        "123456789".to_string(),
        "only in column phone_description".to_string(),
        "only in column address".to_string(),
        "only in column emails".to_string(),
        "only in column urls".to_string(),
        "only in column facebook_urls".to_string(),
        "only in column twitter_handles".to_string(),
        "only in column instagram_handles".to_string(),
        "only in column note".to_string(),
    ];
    for column_value in column_values.iter() {
        let url = format!("{url_api}/contacts?query={column_value}");
        let response = client
            .get(url)
            .send()
            .await
